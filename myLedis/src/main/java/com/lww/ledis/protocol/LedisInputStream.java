 ...
 */
public class LedisInputStream extends FilterInputStream {
    protected final byte[] buf;

    protected int count, limit;
    public LedisInputStream(InputStream in, int size) {
        super(in);
        if (size <= 0) {
    ...
    public LedisInputStream(InputStream in) {
        this(in, 8192);    //8M
    }
    public byte readByte() throws LedisConnectionException {
        ensureFill();
        return buf[count++];
    }
    ...
    public String readLine() {
        ...
        if (reply.length() == 0) {
            throw new LedisConnectionException("服务器已经关闭！");
        }
    ...
    public long readLongCrLf() {
        ...
        while (true) {
            ...
            if (b == '\r') {
                ...
                if (buf[count++] != '\n') {
                    throw new LedisConnectionException("\\r\\n出错了，查看是否以\\r\\n结尾！");
                }
    ...
    @Override
    public int read(byte[] b, int off, int len) throws LedisConnectionException {
        ensureFill();
     ...
     */
    private void ensureFill() throws LedisConnectionException {
        if (count >= limit) {
            try {
                limit = in.read(buf);
                count = 0;
                if (limit == -1) {
                    throw new LedisConnectionException("Unexpected end of stream.");
                }
            } catch (IOException e) {
